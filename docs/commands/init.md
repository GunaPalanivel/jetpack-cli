...
Initialize a new developer environment from a remote repository manifest or local directory.
...
1.  Fetches the `.onboard.yaml` manifest from the specified GitHub repository (or local if using `.` with `--copilot-generate`).
...
| `--copilot-generate` | Generate a manifest from the current directory using Copilot. Use with `.` as repo. | `false` |
...
**Generate manifest from local code (AI):**
```bash
